...
public class PreparedPacketFactory {
    ...
    static {
        ...
        try {
            ...
            DIRECT_BYTEBUF_PREFERRED_FOR_COMPRESSOR = (bufferType.equals(BufferPreference.DIRECT_PREFERRED) || bufferType.equals(BufferPreference.DIRECT_REQUIRED));
            compressor.close();
    ...
    public PreparedPacketFactory(PreparedPacketConstructor constructor, StateRegistry stateRegistry, boolean enableCompression, int compressionLevel, int compressionThreshold, boolean saveUncompressed, boolean releaseReferenceCounted, boolean compatibilityMode) {
        this(constructor, stateRegistry, enableCompression, compressionLevel, compressionThreshold, saveUncompressed, releaseReferenceCounted, compatibilityMode, new PooledByteBufAllocator());
    }
    ...
    private MinecraftCompressorAndLengthEncoder getThreadLocalCompressionEncoder() {
        return compressionEncoder.computeIfAbsent(Thread.currentThread(), key -> new MinecraftCompressorAndLengthEncoder(compressionThreshold, Natives.compress.get().create(compressionLevel)));
    }
