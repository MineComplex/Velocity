...
public class BitStorage {
  ...
  static int fixBitsPerEntry(ProtocolVersion version, int bitsPerEntry) {
    if (bitsPerEntry < 4) {
      ...
      return bitsPerEntry;
    } else if (version.lessThan(ProtocolVersion.MINECRAFT_1_16_4)) {
      return 14;
    } else {
      return 15;
    }
  ...
  public BitStorage(int bitsPerEntry, int size, long[] data) {
    ...
    int expectedLength = (size + valuesPerLong - 1) / valuesPerLong;
    if (data != null) {
  ...
  public void set(int index, int value) {
    if (index < 0 || index > size - 1) {
      ...
      data[cellIndex] = data[cellIndex] & ~(maxValue << bitIndex) | ((long) value & maxValue) << bitIndex;
    }
  }
  ...
  public void write(Object object, ProtocolVersion version) {
    Preconditions.checkArgument(object instanceof ByteBuf);
  ...
  public BitStorage copy() {
    return new BitStorage(bitsPerEntry, size, Arrays.copyOf(data, data.length));
  }
